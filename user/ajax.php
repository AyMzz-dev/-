...
if ($_GET['mod'] === 'login') {
    ...
    if (!$result = $db->select_first_row('sq_agent', '*', array('username' => $_POST['username'], 'password' => $_POST['password']), 'AND')) {
        ...
        $_SESSION['agent_levelid'] = $result['levelid'];
...
switch ($_GET['mod']) {
    ...
    case 'updatelevel':
        ...
        if (!$levelinfo_zsye = $db->select_first_row('sq_level', '*', array('ID' => $result['levelid']), 'AND')) {
            die('等级信息获取失败');
        }
        if (!$agentinfo = $db->select_first_row('sq_agent', '*', array('username' => $_SESSION['agent_username']), 'AND')) {
            die('代理信息获取失败');
        ...
        $ls_ye=bcsub($levelinfo['price'],$levelinfo_zsye['price'],2);
        if($ls_ye<0){
            die('该等级过低不符合你要升级的等级');
        }
        if($ls_ye==0){
        ...
        $newmoney = $agentinfo['money'] - $ls_ye;
    ...
    case 'buyagent':
        ...
        if (empty($_POST['username']) || empty($_POST['pass']) || empty($_POST['qq'])) {
            die('请填写完整信息');
        }
        if (!$levelinfo = $db->select_first_row('sq_level', '*', array('ID' => $_POST['leval']), 'AND')) {
            die('等级信息获取失败，无法继续开通！');
        ...
        if (!$db->update('sq_agent', array('username' => $_SESSION['agent_username']), 'AND', array('money' => $newmoney))) {
            ...
            if (!$db->insert_back_id('sq_agent', array('username' => $_POST['username'], 'password' => $_POST['pass'], 'qq' => $_POST['qq'], 'begintime' => time(), 'levelid' => $_POST['leval'], 'status' => '1', 'superior' => $_SESSION['agent_id'], 'money' => $levelinfo['zsye']))) {
...
//        }
    ...
    case 'leveltips':
        ...
        die('<center>代理应用：' . $appinfo . '，等级名称：' . $levelinfo['lname'] . '，自动补差价升级金额：' . $ls_ye . '，拿货折率：' . $levelinfo['fracture'] . '<br>注意：升级后当前等级会被释放，而不是同时有两个等级，如果希望有多个等级请联系站长新开代理账号！</center>');
        break;
    case 'addagenttips':
        include '../function/function_app.php';
        if (!$agentinfo = $db->select_first_row('sq_agent', '*', array('username' => $_SESSION['agent_username']), 'AND')) {
        ...
        die('代理应用：' . $appinfo . '，等级名称：' . $levelinfo['lname'] . '，开通金额：' . $levelinfo['price'] . '，赠送余额：' . $levelinfo['zsye'] . '，拿货折率：' . $levelinfo['fracture'] . '<br>您当前代理等级开通下级折率：' . $agentlevelinfo['discount'] . '，实际支付：<font color="red">' . ($levelinfo['price'] * $agentlevelinfo['discount']) . '元</font>');
        break;
    case 'changepassword':
