...
switch ($_GET['mod']) {
    ...
    case 'loadbuylist':
        ...
        foreach ($result as $item) {
            ...
            $info .= '<div class="app-card">
  <div class="app-card-header" style="background: linear-gradient(135deg, #3498db, #2c3e50); color: white; padding: 15px 20px; display: flex; justify-content: space-between; align-items: center;">
    <div class="app-name" style="font-size: 1.3rem; font-weight: 600;">' . $item['fidname'] . '</div>
    <div class="app-price" style="font-size: 1.5rem; font-weight: 700;">' . $item['buyprice'] . '元</div>
  </div>
  <div class="app-card-body" style="padding: 20px;">
    <ul class="app-features" style="list-style: none; margin-bottom: 20px;">
      <li style="padding: 8px 0; display: flex; align-items: center; gap: 10px; border-bottom: 1px solid #eee;"><i class="fas fa-tag" style="color: #00b09b;"></i> 购买价格：' . $item['buyprice'] . '元</li>
      <li style="padding: 8px 0; display: flex; align-items: center; gap: 10px; border-bottom: 1px solid #eee;"><i class="fas fa-clock" style="color: #00b09b;"></i> ' . $numtip . '</li>
      <li style="padding: 8px 0; display: flex; align-items: center; gap: 10px; border-bottom: 1px solid #eee;"><i class="fas fa-cube" style="color: #00b09b;"></i> 应用名称：' . $appinfo['appname'] . '</li>
    </ul>
    <div class="app-actions" style="display: flex; justify-content: space-between; gap: 10px;">
      <button class="btn btn-secondary" onclick="info(' . $item['appid'] . ')">查看详情</button>
      <button class="btn btn-primary" onclick="buy(' . $item['ID'] . ')">立即购买</button>
      <button class="btn btn-info" onclick="window.location.href=\'' . $appinfo['upurl'] . '\'">下载应用</button>
    </div>
  </div>
</div>';
...
';
    ...
    case 'queryauth':
        ...
        $info = $db->select_all_row('sq_user', 'uqq,rqq,rtime,balance,appid,aid,mac', array('uqq' => $_POST['qq'], 'rqq' => $_POST['qq']), 'OR');
        if (!is_array($info) || count($info) == 0) {
    ...
    case 'macauth':
        ...
        $info = $db->select_all_row('sq_user', 'mac,uqq,rtime,balance,appid,aid', array('mac' => $_POST['mac']), 'OR');
        include 'function/function_app.php';
        if (!is_array($info) || count($info) == 0) {
            die('设备码【' . $_POST['mac'] . '】没有查询到任何授权信息');
        }
        foreach ($info as $qqinfo) {
            echo '设备码：' . $qqinfo['mac'] . '<br>';
            echo '主人QQ：' . $qqinfo['uqq'] . '<br>';
            echo '开通时间：' . Get_Date($qqinfo['rtime']) . '<br>';
            $appinfo = app_idgetinfo($qqinfo['appid']);
            echo '所属应用：' . $appinfo['appname'] . '<br>';
            if ($appinfo['usetype'] == 'dqsj') {
        ...
        $info = $db->select_all_row('sq_user', 'username,uqq,rtime,balance,appid,aid', array('username' => $_POST['km']), 'OR');
        include 'function/function_app.php';
        if (!is_array($info) || count($info) == 0) {
            die('卡密【' . $_POST['km'] . '】没有查询到任何授权信息');
        }
        foreach ($info as $qqinfo) {
            echo '卡密：' . $qqinfo['username'] . '<br>';
            echo '主人QQ：' . $qqinfo['uqq'] . '<br>';
            echo '开通时间：' . Get_Date($qqinfo['rtime']) . '<br>';
            $appinfo = app_idgetinfo($qqinfo['appid']);
            echo '所属应用：' . $appinfo['appname'] . '<br>';
