...
if ($_GET['mod'] === 'login'){
    ...
    $admin_info = $db->select_first_row('sq_admin_1','*',array('username'=>$_POST['username']),'AND');
    if (!$admin_info){
    ...
    if (verify_password($_POST['password'], $admin_info['password'])){
        ...
        $db->update('sq_admin_1',array('username'=>$_POST['username']),'AND',array('password'=>hash_password($_POST['password'])));
        $login_ok = true;
    }
    if (!$login_ok){
    ...
    $db->update('sq_admin_1',array('username'=>$_SESSION['admin_username']),'AND',array('loginip'=>get_real_ip(),'logintime'=>time()));
    ...
    if (!$result = $db->select_first_row('sq_admin_1','*',array('ID'=>$_SESSION['admin_id'],'username'=>$_SESSION['admin_username']),'AND')){
...
switch ($_GET['mod']){
    ...
    case 'delfid':
        ...
        break;
    case 'addgrade':
        if(!$db->insert_back_id('sq_level',array('lname'=>$_POST['grade_name'],'appid'=>$_POST['app'],'fracture'=>$_POST['grade_present'],'price'=>$_POST['grade_price'],'discount'=>'1','zsye'=>$_POST['grade_zsye']))){
            die('添加失败：'.$db->geterror());
        }else{
            die('代理级别添加成功！');
        }
...
//        }
    ...
    case 'savesiteinfo':
        ...
        break;
    case 'addadmin':
        if (empty($_POST['addusername']) || empty($_POST['adduserpass']) || empty($_POST['adduserqq']) || empty($_POST['addusertype'])){
            die('数据不能为空，请检查');
        }
        if($_POST['addusertype']=='1'){
            $db->insert_back_id('sq_admin_1',array('username'=>$_POST['addusername'],'password'=>md5($_POST['adduserpass']),'qq'=>$_POST['adduserqq']));
            die('管理员新增成功，如果需要删除请进数据库中的表sq_admin_1进行删除');
        } elseif($_POST['addusertype']=='2'){
            $db->insert_back_id('sq_admin_2',array('username'=>$_POST['addusername'],'password'=>md5($_POST['adduserpass']),'qq'=>$_POST['adduserqq']));
            die('管理员新增成功，如果需要删除请进数据库中的表sq_admin_2进行删除');
        }
        die('请正确填写类型');
        break;
    case 'sendtestmail':
    ...
    case 'changepassword':
        ...
        if (!$agentinfo = $db->select_first_row('sq_admin_1','*',array('username'=>$_SESSION['admin_username']),'AND')){
            die('管理员信息获取失败');
        }
        ...
        if(!$db->update('sq_admin_1',array('username'=>$_SESSION['admin_username']),'AND',array('password'=>md5($_POST['newpass'])))){
...
';
    ...
    case 'resettoken':
        $token = rand_str(64);
        $result = $db->update('sq_admin_1',array('ID'=>$_SESSION['admin_id']),'AND',array('accesstoken'=>$token));
        die($token);
        break;
    case 'getbclist':
